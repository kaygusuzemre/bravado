...
export default function (router, db, cache) {
  /**
     ...
     */
  router.post("/challenge/all", utils(db, cache).restrictByUserRole("user"), (req, res) => {
    let sql = "SELECT * FROM challenge ";
    const {sort, filter, search} = req.body;
    let now = new Date().toMysqlFormat();
    if (filter === "FINISHED") 
      sql += `WHERE finishDate>="${now}" `;
  ...
  /**
     ...
     *    string imageURL
     */
  router.post("/challenge/add", utils(db, cache).restrictByUserRole("user"), (req, res) => {
    const {
      title,
      categories,
      startDate,
    ...
    if (validator.isEmpty(validator.trim(title))) 
      res.json({status: "error", key: "title", msg: "title cannot be empty"});
    else if (categories.length < 1) 
      res.json({status: "error", key: "categories", msg: "at least one category is needed."});
    else if (!validator.matches(String(startDate), /^([0-9]{2,4})-([0-1][0-9])-([0-3][0-9])(?:( [0-2][0-9]):([0-5][0-9]):([0-5][0-9]))?$/)) 
      res.json({status: "error", key: "startDate", msg: "startDate is not valid."});
    else if (!validator.matches(String(finishDate), /^([0-9]{2,4})-([0-1][0-9])-([0-3][0-9])(?:( [0-2][0-9]):([0-5][0-9]):([0-5][0-9]))?$/)) 
      res.json({status: "error", key: "finishDate", msg: "finishDate must be in valid interval."});
      ...
      res.json({status: "error", key: "description", msg: "description cannot be empty"});
    else if (!validator.isHexColor(validator.trim(badge.color))) 
      res.json({status: "error", key: "badge", msg: "color is not hex "});
    else 
      db.query("INSERT INTO challenge (owner,title,startDate,finishDate,goal,content,reward) VALUES (?,?,?,?,?,?,?)", [
        req.user.userId,
        title,
        startDate,
        finishDate,
  ...
  /**
     ...
     * @param int   id;
     */
  router.get("/challenge/get/:id", utils(db, cache).restrictByUserRole("user"), (req, res) => {
    db.query(`SELECT * FROM challenge WHERE challengeId=?`, [req.params.id], function (error, results, fields) {
      if (error) 
        res.json({status: "error", msg: "Unknown error", error: error});
      else 
        res.json(results);
  ...
  /**
     ...
     * @param int   userId
     */
  router.post("/challenge/join", utils(db, cache).restrictByUserRole("user"), (req, res) => {
    db.query(`INSERT INTO progress (challengeId,userId,status) VALUES (?,?,?)`, [
  ...
  /**
     ...
     * @param int   userId
     */
  router.post("/challenge/quit", utils(db, cache).restrictByUserRole("user"), (req, res) => {
    db.query(`UPDATE progress SET status='gaveUp' WHERE challengeId=? AND  userId=?`, [
  ...
  router.get("/challenge/participants/:id", utils(db, cache).restrictByUserRole("user"), (req, res) => {
    db.query(`
